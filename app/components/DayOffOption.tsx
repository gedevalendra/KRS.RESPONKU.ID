...
export default function DayOffOption({ dayOffSettings, onToggle, onChangePreferredDay }: DayOffOptionProps) {
  ...
  return (
    <div className="border border-blue-100 bg-blue-50/50 rounded-lg p-3.5">
      <label className="flex items-start gap-2.5 cursor-pointer">
        ...
        <span className="text-sm">
          <span className="font-semibold flex items-center gap-1.5 text-slate-800">
            <CalendarOff className="w-3.5 h-3.5 text-blue-600" /> Sisakan satu hari libur penuh
          </span>
      ...
      {settings.enabled && (
        <div className="mt-3 pl-6">
          ...
          <select
            ...
            className="border border-slate-200 rounded-md px-3 py-2 text-sm outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent bg-white cursor-pointer"
          >
            {DAY_LABELS.map((d) => (
